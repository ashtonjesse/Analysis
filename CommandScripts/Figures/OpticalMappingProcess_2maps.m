...
% sSavePath2 = 'C:\Users\jash042.UOA\Dropbox\Publications\2015\Paper1\Figures\OpticalMappingProcess.bmp';
sFileSavePath = 'D:\Users\jash042\Documents\PhD\Thesis\Figures\Working\OpticalMappingProcess_#';
...
% oLine = plot(oAxes,[aTime(1) aTime(end)],[oElectrode2.Processed.Data(oElectrode2.arsps.RangeEnd(iBeat,1)) ...
%     oElectrode2.Processed.Data(oElectrode2.arsps.RangeEnd(iBeat,1))],'r--','linewidth',1);
% set(get(get(oLine,'Annotation'),'LegendInformation'),'IconDisplayStyle','off');
% %plot activation time
% oLine = plot(oAxes,[aBeatTime(oElectrode2.arsps.Index(iBeat)) aBeatTime(oElectrode2.arsps.Index(iBeat))],...
%     [YLim(1)+40 oElectrode2.Processed.Data(oElectrode2.arsps.RangeEnd(iBeat,1)-5)],'r--');
% set(get(get(oLine,'Annotation'),'LegendInformation'),'IconDisplayStyle','off');
% oLine = scatter(oAxes,aBeatTime(oElectrode2.arsps.Index(iBeat)), aBeatData(oElectrode2.arsps.Index(iBeat)),36,'r','filled');
% set(get(get(oLine,'Annotation'),'LegendInformation'),'IconDisplayStyle','off');
% %label activation time
% text(aBeatTime(oElectrode2.arsps.Index(iBeat)-2), aBeatData(oElectrode2.arsps.Index(iBeat)),'dF/dt max',...
%     'horizontalalignment','right','parent',oAxes,'color','r');
% text([XLim(1)+0.105],aBeatData(oElectrode2.arsps.Index(iBeat)),'Atrial','color','r','parent',oAxes,'horizontalalignment','center','rotation',90);
% 
% %plot electrode 1
...
% set(oChildren(5),'color','b');
% set(oChildren(2),'color','r');
% %create time scale
...
% 
% %% create labels
% annotation('textbox',[0,0.9,0.1,0.1],'string','A','linestyle','none','fontweight','normal','color','k','fontsize',14);
% annotation('textbox',[0.5,0.9,0.1,0.1],'string','B','linestyle','none','fontweight','normal','color','k','fontsize',14);
% annotation('textbox',[0,0.48,0.1,0.1],'string','C','linestyle','none','fontweight','normal','color','k','fontsize',14);
% annotation('textbox',[0.5,0.48,0.1,0.1],'string','D','linestyle','none','fontweight','normal','color','k','fontsize',14);
% 
...
% export_fig(strrep(sFileSavePath, '#', '1'),'-png','-r600','-nocrop','-transparent','-painters');%
...
%plot the schematic
oImage = imshow('D:\Users\jash042\Documents\DataLocal\Imaging\Prep\20140722\SchematicForFigure2.bmp','Parent', oOverlay, 'Border', 'tight');
...
set(oTitle,'string',['SAN component',10,'AP50%'],'fontweight','bold');
set(oTitle,'position',get(oTitle,'position') - [0 1 0]);

% [figx figy] = dsxy2figxy(oPointAxes, [aAxesCoords(1,1)-0.5;aAxesCoords(1,1)-0.05], ...
%     [aAxesCoords(2,1)-1.5;aAxesCoords(2,1)+0.2]);
% annotation('textarrow',figx,figy,'string',['SVC-IVC',10,'axis (mm)'],'headstyle','plain','headwidth',4,'headlength',4,'color','k','horizontalalignment','right');
% % [figx figy] = dsxy2figxy(oPointAxes, [aAxesCoords(1,1)+0.4;aAxesCoords(1,1)-0.05], ...
% %     [aAxesCoords(2,1)-1;aAxesCoords(2,1)-0.11]);
...
scatter(oPointAxes, oElectrode1.Coords(1,:), oElectrode1.Coords(2,:), ...
    'sizedata',49,'Marker','o','MarkerEdgeColor','k','MarkerFaceColor','none');
text(oElectrode1.Coords(1,:), oElectrode1.Coords(2,:),'1','color','k','parent',oPointAxes,'fontsize',6,'horizontalalignment','center');
hold(oPointAxes,'on');
scatter(oPointAxes, oElectrode2.Coords(1,:), oElectrode2.Coords(2,:), ...
    'sizedata',49,'Marker','o','MarkerEdgeColor','w','MarkerFaceColor','none');
text(oElectrode2.Coords(1,:), oElectrode2.Coords(2,:),'2','color','w','parent',oPointAxes,'fontsize',6,'horizontalalignment','center');
...
axis(oPointAxes,'off');
oTitle = get(oAxes,'title');
set(oTitle,'string',['Atrial component',10,'dF/dt max'],'fontweight','bold');
set(oTitle,'position',get(oTitle,'position') - [0 1 0]);
...
cbarf_edit(aCRange, aContours,'horiz','nonlinear',oAxes,'AT',10);
aCRange = [0 12];
...
export_fig(strrep(sFileSavePath, '#', '2'),'-bmp','-r600','-nocrop');%
% % %crop the images
sChopString = strcat('D:\Users\jash042\Documents\PhD\Analysis\Utilities\convert.exe', {sprintf(' %s.bmp',strrep(sFileSavePath, '#', '2'))}, ...
    {sprintf(' %s.png',strrep(sFileSavePath, '#', '1'))}, {' -gravity center -composite'},{sprintf(' %s.bmp', strrep(sFileSavePath, '_#', ''))});
