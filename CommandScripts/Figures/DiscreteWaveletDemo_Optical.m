...
aBeatRange = oOptical.Beats.Indexes(iBeat,1):...
    oOptical.Beats.Indexes(iBeat,2);
aBeatTimes = [oOptical.TimeSeries(aBeatRange(1)), oOptical.TimeSeries(aBeatRange(end))];
%% plot top panel
%plot original (processed) data
oAxes = oSubplotPanel(1,1).select();
aData = oElectrode.Processed.Data;
aTime = oOptical.TimeSeries;
iStart = 1;
iEnd = numel(aTime)/2;
...
rectangle('Position',[aBeatTimes(1)-(aBeatTimes(2)-aBeatTimes(1))*3/2 oYLim(1)+(oYLim(2)-oYLim(1))/2 (aBeatTimes(2)-aBeatTimes(1))*3 (oYLim(2)-oYLim(1))/2],...
    'parent',oAxes,'edgecolor','k','erasemode','xor');

%plot discrete wavelet decomposition
oAxes = oSubplotPanel(1,2).select();
aData = oOptical.FilterData(oElectrode.Processed.Data, 'DWTFilterRemoveScales', iScaleToRemove);
aTime = oOptical.TimeSeries;
hline = plot(oAxes,aTime(iStart:iEnd), aData(iStart:iEnd), 'k');
set(hline,'linewidth',1);
%set axes colour
...
aData = oOptical.FilterData(oElectrode.Processed.Data, 'DWTFilterKeepScales', iScaleToRemove);
aTime = oOptical.TimeSeries;
hline = plot(oAxes,aTime(iStart:iEnd), aData(iStart:iEnd), 'k');
set(hline,'linewidth',1);
%set axes colour
...
aTime = oOptical.TimeSeries(aBeatRange);
oXLim = [aBeatTimes(1) (aBeatTimes(2)-aBeatTimes(1))/2+aBeatTimes(1)];
aData = oElectrode.Processed.Data(aBeatRange);
...
aData = oOptical.CalculateSlope(oElectrode.Processed.Data,5,3);
...
oOverlay = axes('position',get(oAxes,'position'));
aData = oOptical.FilterData(oElectrode.Processed.Data, 'DWTFilterKeepScales', iScaleToRemove);
aThisData = aData(aBeatRange);
aTime = oOptical.TimeSeries(aBeatRange);
hline3 = plot(oOverlay,aTime, aThisData, 'r');
...
%plot slope
oOverlay = axes('position',get(oAxes,'position'));
aData = oOptical.CalculateSlope(aData,5,3);
hline4 = plot(oOverlay,aTime, aData(aBeatRange), 'r--','displayname','Gradient');
