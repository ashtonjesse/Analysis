...
oOnsetAxes = aSubplotPanel(1,1).select();
...
set(get(oOnsetAxes,'title'),'string','Chemoreflex','fontweight','bold');
% %add panel label
text(axlim(1)-200,aylim(2)+1.5,'A','parent',oOnsetAxes,'fontsize',12,'fontweight','bold');
oLegend = legend(oOnsetAxes,'Pre-IVB','Post-IVB','location','northeast');
set(oLegend,'position', [0.1121    0.8395    0.2933    0.1005]);
...
axis(oOverlay,'off');
text(axlim(1)-200,aylim(2)+1.5,'B','parent',oOverlay,'fontsize',12,'fontweight','bold');
%get data
