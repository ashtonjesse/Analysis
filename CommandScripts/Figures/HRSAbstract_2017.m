...
% aData = oThisPressure.oPhrenic.Electrodes.Processed.Data ./ ...
%     (oThisPressure.oExperiment.Phrenic.Amp.OutGain*1000)*10^6;
aData = oThisPressure.oPhrenic.Electrodes.Processed.Integral(25000:end-25000)/1000;
aTime = oThisPressure.oPhrenic.TimeSeries(25000:end-25000);
...
set(oXlabel,'position',oPosition + [0 0.1 0]);
    ...
    for j = 1:xrange
        ...
        if ~isempty(aOriginCoords)
            scatter(oOriginAxes, aOriginCoords(1,:), aOriginCoords(2,:), ...
                'sizedata',49,'Marker','p','MarkerEdgeColor','w','MarkerFaceColor','k');%size 6 for posters
        end
...
cbarf_edit(aCRange, aContours,'vertical','nonlinear',oAxes,'AT',5);
aCRange = [0 12];
oLabel = text(-3.8,(aCRange(2)-aCRange(1))/2,'Atrial AT (ms)','parent',oAxes,'fontunits','points','fontweight','normal','horizontalalignment','center','rotation',90);
set(oLabel,'fontsize',6);
