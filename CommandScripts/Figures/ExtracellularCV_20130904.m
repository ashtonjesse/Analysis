...
for k = iStartBeat:1:iStartBeat+(xrange*yrange)-1
    ...
    aCVdata = oActivation.Beats(k).CVApprox(idxCV);
    bplot(aCVdata,oAxes,iBoxXLocation,'nolegend','nooutliers','linewidth',1);
...
for i = 1:5
    for j = 1:5
        ...
        if (i == 1) && (j == 1)
            ...
            axis(oAxes,'tight');
            axis(oAxes,'off');
            idxCV = find(~isnan(oActivation.Beats(iBeat).CVApprox));
            aCVdata = oActivation.Beats(iBeat).CVApprox(idxCV);
            aCVdata(aCVdata > 1) = 1;
            scatter(oOverlay,oActivation.CVx(idxCV),oActivation.CVy(idxCV),10,aCVdata,'filled');
            hold(oOverlay, 'on');
            quiver(oOverlay,oActivation.CVx(idxCV),oActivation.CVy(idxCV),oActivation.Beats(iBeat).CVVectors(idxCV,1), ...
                oActivation.Beats(iBeat).CVVectors(idxCV,2),'color','k','linewidth',0.6);
            hold(oOverlay, 'off');
            caxis(aCVRange);
