...
aMeanBaselineRate = cell(size(aControlPressureData));
aMeanPlateauPressure = cell(size(aControlPressureData));
for i = 1:numel(aControlPressureData)
    ...
    aMeanBaselineRate{i} = zeros(numel(aCellArray),1);
    aMeanPlateauPressure{i} = zeros(numel(aCellArray),1);
    for j = 1:numel(aCellArray)
        ...
        aMeanBaselineRate{i}(j) = mean(oPressure.Baseline.BeatRates);
        aMeanPlateauPressure{i}(j) = mean(oPressure.Plateau.BeatPressures);
    end
end
