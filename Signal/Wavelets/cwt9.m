function coefs = cwt9(SIG,scales,WAV,plotmode,xlim)
...
if ischar(WAV)
    precis = 10; % precis = 15;
    [val_WAV,xWAV] = intwave9(WAV,precis);
    stepWAV = xWAV(2)-xWAV(1);
    wtype = wavemngr9('type',WAV);
...
elseif iscell(WAV)
    ...
    elseif ischar(WAV{1})
        precis  = 10;
        val_WAV = intwave9(WAV{1},precis);
        wtype = wavemngr9('type',WAV{1});        
        getINTEG = 0;
        getWTYPE = 0;
    end
    xATTRB  = WAV{2};
...
for k = 1:nb_SCALES
    ...
    f            = fliplr(val_WAV(j));
    coefs(ind,:) = -sqrt(a)*wkeep9(diff(wconv9(val_SIG,f)),lenSIG);
    ind          = ind+1;
end
