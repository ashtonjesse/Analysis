...
% Check arguments.
wname = deblankl9(wname);
debut = wname(1:2);

[wtype,fname,family,bounds] =  ...
    wavemngr9('fields',wname,'type','file','fn','bounds');
...
elseif  nargin == 2
    ...
    pflag = 1;
    if  ischar(in2)
        if ischar(in3) , iter = 8; else , iter = in3; end
    else
        if ischar(in3)
            iter = in2;
        else
...
switch wtype
    ...
    case 2
      [Lo_D1,Hi_D2,Lo_R2,Hi_R1,Lo_D2,Hi_D1,Lo_R1,Hi_R2] = wfilters(wname);

      mul = 1;
      isBior = wavemngr9('isbior',wname);
      if isBior
          Nr = wstr2num(wname(5));
