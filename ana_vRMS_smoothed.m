function [vrms, slopes1, slopes2, curvature] = ana_vRMS_smoothed
...
smoothed = zeros(sd1,sd2);

fprintf('MovingAve');
for k = 1:sd2
    if ~mod(k,sd2/5); fprintf('.'); end;
    smoothed(:,k) = smooth(DATA.Unemap.Pot.OrigBase(:,k));

end
fprintf('Loess');
for k = 1:sd2
    if ~mod(k,sd2/5); fprintf('.'); end;
    smoothed(:,k) = smooth(smoothed(:,k),'loess');
end

for k = 1:sd1
    vrms(k) = sqrt(sum(smoothed(k,:).^2) / sd2);
end
order = 20;
slopes1 = ana_movingslope(vrms,order,5);
slopes2 = ana_movingslope(slopes1,order,5);
