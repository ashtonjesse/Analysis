function out = scatplot(x,y,method,radius1,radius2,N,n,po,ms,oAxes)
...
if nargin<5 | isempty(radius1)
    radius1 = sqrt(((max(x)-min(x))/30)^2 + ((max(y)-min(y))/30)^2);
end
if nargin<6 | isempty(radius2)
    if strcmpi(method,'square')
        radius2 = radius1;
    end
end
if nargin<7 | isempty(po)
    po = 1; %plot option
end
if nargin<8 | isempty(ms)
    ms = 4; %markersize
end
if nargin<9 | isempty(N)
    N = 100; %length of grid
end
if nargin<10 | isempty(oAxes)
    oAxes = gca;
end
...
dd = datadensity(x,y,method,radius1,radius2);
%------------- Gridding -------------------
...
return
%~~~~~~~~~~ Data Density ~~~~~~~~~~~~~~
function dd = datadensity(x,y,method,r1,r2)
...
switch method %Calculate Data Density
    case 'sq'  %---- Using squares ----
        for k=1:Ld
            dd(k) = sum( x>(x(k)-r1) & x<(x(k)+r1) & y>(y(k)-r2) & y<(y(k)+r2) );
        end %for
        area = (2*r1)*(2*r2);
        dd = dd/area;
    case 'ci'
        for k=1:Ld
            dd(k) = sum( sqrt((x-x(k)).^2 + (y-y(k)).^2) < r1 );
        end
        area = pi*r1^2;
        dd = dd/area;
...
return
%~~~~~~~~~~ Graf Scatter Plot ~~~~~~~~~~~
function varargout = gsp(x,y,c,ms,oAxes)
...
for k=1:size(map,1) 
    if any(ind==k)
        h(end+1) = line('Xdata',x(ind==k),'Ydata',y(ind==k), ...
            ...
            'Marker','.','MarkerSize',ms,'parent',oAxes);
    end
end
if nargout==1
    varargout{1} = h; 
